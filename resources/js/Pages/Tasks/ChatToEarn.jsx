...
export default function ChatToEarn({ stats, pay_per_message, cost_per_message, credits }) {
    ...
    useEffect(() => {
        let timer;
        if (modalState === 'chatting' && timeLeft > 0) {
    ...
    const handleCompleteTask = () => {
        ...
        router.post(route('chat-to-earn.complete'), { message_count: userMessageCount }, {
            preserveScroll: true,
            onSuccess: () => { setModalState('hidden'); setIsSubmitting(false); },
            onError: () => { alert("Failed to save task."); setIsSubmitting(false); }
        });
    };
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/90 dark:bg-black/80 backdrop-blur-sm p-4 sm:p-6">
                    <div className="bg-white dark:bg-[#11071c] w-full max-w-md h-full max-h-[600px] rounded-2xl border border-gray-200 dark:border-purple-500/30 shadow-2xl flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center py-1.5 px-4 bg-gray-50 dark:bg-[#150a21]">
                            <span className="text-[10px] text-amber-500 font-bold">Credits: {localCredits}</span>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gray-100 dark:bg-[#0d0415]">
                            ...
                            {isBotTyping && <div className="text-gray-500 text-xs italic">Partner is typing...</div>}
                        ...
                        <div className="p-4 bg-white dark:bg-[#11071c] border-t border-gray-200 dark:border-gray-800 relative z-10">
                            {localCredits < cost_per_message ? (
                                <div className="text-center text-rose-500 text-xs font-bold mb-3 bg-rose-500/10 py-2 rounded">Out of credits! Claim your earnings and recharge.</div>
                            ) : (
                                <form onSubmit={handleSendMessage} className="flex items-center gap-2 mb-3">
                                    <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type your message..." disabled={timeLeft <= 0 || isSubmitting} className="flex-1 bg-gray-50 dark:bg-[#1a0e29] border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:ring-1 focus:ring-fuchsia-500 focus:border-fuchsia-500" />
                                    <button type="submit" disabled={!inputText.trim() || timeLeft <= 0 || isSubmitting} className="bg-indigo-600 hover:bg-indigo-500 text-white w-12 h-12 rounded-xl flex items-center justify-center transition disabled:opacity-50">📤</button>
                                </form>
                            )}

                            <button onClick={handleCompleteTask} disabled={userMessageCount === 0 || isSubmitting} className={`w-full py-3.5 rounded-xl font-bold text-sm transition-all flex justify-center items-center gap-2 ${isSubmitting ? 'bg-emerald-600 opacity-70 text-white cursor-wait' : userMessageCount > 0 ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-gray-100 dark:bg-[#1a0e29] text-gray-400 dark:text-gray-500 cursor-not-allowed'}`}>
